...
MODEL_PATH = os.environ.get("MODEL_PATH", "/models/FireRedAudio")
VAE_DECODER_PATH = os.environ.get(
...
def _load_engine():
    ...
    with _engine_lock:
        ...
        engine.vae_decoder = PretrainedRedAEAudioDecoderV1.from_pretrained(
            config=PretrainedRedAEDecoderConfig(),
            ckpt_path=VAE_DECODER_PATH,
        ).to(DECODER_DEVICE)
        _engine = engine
...
def _save_audio(audio, prefix):
    path = OUTPUT_DIR / f"{prefix}-{uuid.uuid4().hex[:10]}.wav"
    torchaudio.save(str(path), audio.detach().cpu().float(), 24000)
    return str(path)
